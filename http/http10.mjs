...
const handler = (req) => {
    ...
    if (req.url === '/index.html') {
        const bodyBuffer = Buffer.from(`<html>
            <head><title>HTTP 1.0 response</title></head>
            <body>HTTP 1.0 <img src="img.svg"></body>
        </html>`, 'utf-8');
...
server.on('connection', (socket) => {
    // http 1.0
    // GET|POST|HEAD /index.html HTTP/1.0\r\n
    // Headers
    // [opt] Body
    ...
    const requestLineAndHeadersBuffer = Buffer.alloc(4096); // request line + headers
    let offset = 0;
    let checkpoint = 0;
